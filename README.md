...
注意：句意级攻击实现过程中使用了谷歌翻译的调用接口。由于翻译限额的原因，句意级攻击生成的测试集只有30条；如果您有更合适的翻译方法，可在`TextAttack.py`文件中修改`transback_noise`函数。

如果需要本地生成对抗性文本且需要中文词语级的攻击方法，请下载中文近义词工具包`Synonmys`：
https://github.com/chatopera/Synonyms

如果不需要，在`Demo.py`文件中将：
```
import TextAttack
```
注释掉即可。

运行`Demo.py`,按照指定输入信息开始测试。
...
#### CODAH：https://paperswithcode.com/dataset/codah
...
####  Flipkart Product reviews with sentiment Dataset：https://www.kaggle.com/dsv/4940809
...
####  Math23K：https://paperswithcode.com/dataset/math23k
...
####  Translation2019zh: https://aistudio.baidu.com/datasetdetail/209041
